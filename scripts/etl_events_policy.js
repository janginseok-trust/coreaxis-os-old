// scripts/etl_events_policy.js (ESM)
...
async function runPolicyETL() {
  console.log("⚖️ Policy ETL 시작");
  ...
  for (const src of sources) {
    try {
      ...
      for (const item of feed.items.slice(0, 10)) {
        ...
        // 저장 → saveEvent 내부에서 매핑/impact_score 자동 업데이트
        saveEvent(row, text);
        total++;
      }
    } catch (err) {
  ...
  console.log(`✅ Policy ETL 완료: ${total}건`);
  return total;
}
